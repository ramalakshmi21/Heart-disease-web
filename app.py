...
import os, uuid, re, html, tempfile
import numpy as np
import pandas as pd
import matplotlib
matplotlib.use("Agg")
...
UPLOAD_DIR = os.path.join(tempfile.gettempdir(), "heart_disease_generated")
...
def make_outputs(name, df, target, test_size=0.30):
    ...
    if name == "J48 Decision Tree":
        ...
        plt.close(fig)
        tree_url = url_for("generated_file", filename=os.path.basename(tree_path))
...
def run():
    ...
    try:
        ...
        return render_template(
            ...
            cm_url=url_for("generated_file", filename=os.path.basename(cm_path)),
            roc_url=url_for("generated_file", filename=os.path.basename(roc_path)),
